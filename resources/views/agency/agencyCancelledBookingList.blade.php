	...
	<div class="m-content agencies_list">
		...
		<div class="m-portlet m-portlet--mobile">
			...
			<div class="m-portlet__body">
				...
				<!--end: Search Form -->
				<!--begin: Datatable -->
				<table class="m-datatable snoagentbookingnew1" id="html_table" width="100%">
					<thead>
						<tr>
							<th title="S no">
								S No
                    ...
                    <?php $guest = unserialize($agency_booking_detail->guest); ?>
                      <tr>
							...
							<td>{{$guest[0]['firstname'][1]}}</td>
							
							<td>{{$agency_booking_detail->email}}</td>
							
                            <td>{{$agency_booking_detail->user_type}}</td>
                            
                            <td><?php if($agency_booking_detail->booking_confirm == 2) { ?>  Cancel <?php
				   } else { ?>Pending <?php } ?></td>
         ...
         @if(!empty($agency_booking))
  @foreach($agency_booking as $agency_booking_detail)      
<div class="modal modalviewmore fade show modelshow optioniddata<?php echo $agency_booking_detail->id; ?>" id="m_modal_6 popup" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" style="display: none; padding-right: 17px;">

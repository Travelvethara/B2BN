...
<!-- END: Left Aside -->
	...
	<div class="m-content agencies_list">
		...
		<div class="m-portlet m-portlet--mobile">
			...
			<div class="m-portlet__body">
				...
				<table class="m-datatable snoagentbooking" id="html_table" width="100%">
                    ...
                    <?php $guest = unserialize($agency_booking_detail->guest); ?>
                    
                      <tr>
                            <td>{{$sno}}</td>
                             <td><?php echo  date("d M Y", strtotime($agency_booking_detail->Bookdate));?> <?php echo date("h:sa", strtotime($agency_booking_detail->bookingdate));?></td>
                            <td>TP{{$agency_booking_detail->hotelid}}{{$agency_booking_detail->id}}</td>
                           ...
                           <td><?php if($agency_booking_detail->user_type == 'SubAgencyManger') { echo $agencyArray[$agency_booking_detail->login_id]; } elseif($agency_booking_detail->user_type == 'AgencyManger') { echo $agencyArray[$agency_booking_detail->login_id]; }elseif($agency_booking_detail->user_type == 'UserInfo'){ echo $userinformationArray[$agency_booking_detail->login_id]; }  ?></td>
					...
					if($agency_booking_detail->CancellationDeadline >= $dateMy) { ?> <td><span class="m-badge  m-badge--success m-badge--wide"> {{date("d M Y", strtotime ($agency_booking_detail->CancellationDeadline))}}</span></td> <?php
                            ...
                            <td><?php if($agency_booking_detail->booking_confirm == 1) { ?> Paid <?php
				   } else { ?> Unpaid <?php } ?></td>
                   
                            <td>${{$agency_booking_detail->totalprice}}</td>
							
						    <td><button class="btn btn-link" data-id="<?php echo $agency_booking_detail->id; ?>" id="popup">View </button>
                        ...
                        <button class="btn btn-link" data-id="<?php echo $agency_booking_detail->id; ?>" id="popupcancel">Cancel</button>
                        <?php } } ?>
        ...
        <input type="hidden" id="homeurl" value="<?php echo URL::to("/agencyUnBookingdetails");?>"/>
    ...
    <!--  <div class="modal-header">
     <h5 class="modal-title" id="exampleModalLongTitle">Modal title</h5>
        ...
        <!--<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>-->
            ...
            <input type="hidden" name="Customerrole" value="AgencyManger" />
...
<div class="modal modalviewmore fade show modelshow optionid<?php echo $vouched_booking_detail->id; ?>" id="m_modal_6 popup" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" style="display: none; padding-right: 17px;">
     ...
     <?PHP } ?>
	   ...
	   $roomdetails=unserialize($room);
	    ...
	    $roomlistarray = array(); 
		 ...
		 for($i=0;$i<$vouched_booking_detail->no_of_room; $i++){
 ...
 <?php 
    ...
    <?php if($vouched_booking_detail->booking_confirm == 1) {?>
	 ...
	 if($vouched_booking_detail->booking_confirm == 0) {?>
     ...
     <?php } ?>
      ...
      <?php } ?>
        ...
        <form action="" method="post" id="paymentsummit<?php echo $vouched_booking_detail->id; ?>">
            <input type="hidden" name="_token" value="<?php echo @csrf_token(); ?>">
            <input type="hidden" name="rolename" value="<?php echo $Premissions['type']; ?>">
...
<script>
	...
	$(document).on('click', '.Printdocument', function(){
		...
		//alert('fhljklhkjlkghlkjlgkhjklgh');

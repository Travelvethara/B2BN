				...
				<table class="m-datatable snoagentbooking" id="html_table" width="100%">
					<thead>
						<tr>
                           ...
                           <th title="Action" class="actionagentbooking">
								Action
							</th>
					...
					$guest = unserialize($vouched_booking_detail->guest);
					?>
                      <tr>
                            <td>{{$sno}}</td>
					...
					if($vouched_booking_detail->CancellationDeadline >= $dateMy) { ?> <td><span class="m-badge  m-badge--success m-badge--wide"> {{date("d M Y", strtotime ($vouched_booking_detail->CancellationDeadline))}}</span></td> <?php
						    ...
						    <td><button class="btn btn-link" data-id="<?php echo $vouched_booking_detail->id; ?>" id="popup">View </button>
                            <?php 
                        $dateMy = date('Y-m-d');
                        if($vouched_booking_detail->CancellationDeadline >= $dateMy ){ ?>	
                        <button class="btn btn-link" data-id="<?php echo $vouched_booking_detail->id; ?>" id="popupcancel">Cancel</button>
                        <?php } ?>	
...
var location= new google.maps.LatLng(-25.363, 131.044);
...
var infoWindow = new google.maps.InfoWindow({
    ...
    <script async defer
    src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCxaj3N_xEOAM7SnbE90zFQsoI80cAkcEY&callback=initMap">
    </script>